...
from tianshou.data import PrioritizedReplayBuffer, PrioritizedVectorReplayBuffer
...
def main(
    ...
    if num_training_envs > 1:
        buffer = PrioritizedVectorReplayBuffer(buffer_size, len(training_envs), alpha=0.6, beta=0.4)  # 多环境使用优先向量经验池
    else:
        buffer = PrioritizedReplayBuffer(buffer_size, alpha=0.6, beta=0.4)  # 单环境使用优先经验池
    ...
    def update_beta(step: int, beta_anneal_step: int, beta_final: float, beta_initial: float):
        if step > beta_anneal_step:
            current_beta = beta_final
        else:
            current_beta = beta_initial + (beta_final - beta_initial) * (step / beta_anneal_step)
        buffer.set_beta(current_beta)  # 更新 beta
    # Training loop
    if not watch:
        for epoch_num in range(epoch):
            # 每个 epoch 更新一次 beta
            update_beta(epoch_num, beta_anneal_step=100000, beta_final=1.0, beta_initial=0.4)
